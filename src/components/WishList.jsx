...
const WishList = ({ route }) => {
    ...
    const quitarCarrito = (idCarrito) => {
        ...
        axios.get(path)
            .then(function (response) {
                if (response.data === "ok") {
                    Alert.alert("Confirmación", "Su producto ha sido eliminado de la lista");
                } else {
    ...
    return (
        <View style={styles.body}>
            ...
            <ScrollView ref={ref} style={styles.body.grandContainer}>
                <Text style={styles.productoDetalle.tittle}>Lista de deseos {"\n"}
                            ...
                            <View style={{ flexDirection: "row", backgroundColor: "white", borderRadius: 5, marginBottom: 10 }} key={{ index }}>
                                ...
                                <View style={{ width: 140, paddingTop: 30 }}>
                                    <Text style={styles.buttons} onPress={() => {
                                        Alert.alert('Confirmación', 'Usted comprará este producto', [{
                                            text: 'Cancelar',
                                    ...
                                    <Text style={styles.buttons.close} onPress={() => {
                                        Alert.alert('Confirmación', 'Se quitará este producto de su lista de favoritos', [{
                                            text: 'Cancelar',
                                            onPress: () => console.log('Cancel Pressed'),
                                            style: 'cancel',
                                        },
